...
Bedrock の ThrottlingException / ModelNotReadyException のように
「時間をおけば成功しうる」一時的な失敗に対して、
AWS 公式推奨の「指数バックオフ + フルジッター」方式で自動リトライする。

Step Functions から呼ばれる各 Lambda で共有するため lambda_src 直下に置き、
デプロイ時は modules/lambda の archive_file が lambda_function.py と並べて
同じ zip に格納する（実行時は同一ディレクトリに配置される）。
...
使い方:
    response = retry_call(bedrock.invoke_model, modelId=..., body=...)
"""
